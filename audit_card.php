...
$permissiontoread = $user->rights->dolismq->audit->read;
$permissiontoadd = $user->rights->dolismq->audit->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontodelete = $user->rights->dolismq->audit->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->dolismq->audit->write; // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->rights->dolismq->audit->write; // Used by the include of actions_dellink.inc.php
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/dolismq/audit_list.php', 1);

	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) $backtopage = $backurlforlist;
			else $backtopage = dol_buildpath('/dolismq/audit_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Audit")), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->dolismq->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $user->rights->dolismq->audit->read; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->dolismq->audit->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('dolismq:Audit', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlright = '<a href="'.dol_buildpath('/dolismq/audit_agenda.php', 1).'?id='.$object->id.'">';
	...
	$diroutput = $conf->dolismq->dir_output;
	$trackid = 'audit'.$object->id;
