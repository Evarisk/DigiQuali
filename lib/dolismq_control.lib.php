...
function controlPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/dolismq/control_card.php", 1).'?id='.$object->id;
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private']))
	{
		...
		$head[$h][0] = dol_buildpath('/dolismq/control_note.php', 1).'?id='.$object->id;
	...
	$head[$h][0] = dol_buildpath("/dolismq/control_document.php", 1).'?id='.$object->id;
	$head[$h][1] = $langs->trans('Documents');
	if (($nbFiles + $nbLinks) > 0) $head[$h][1] .= '<span class="badge marginleftonlyshort">'.($nbFiles + $nbLinks).'</span>';
	$head[$h][2] = 'document';
	$h++;

	$head[$h][0] = dol_buildpath("/dolismq/control_agenda.php", 1).'?id='.$object->id;
