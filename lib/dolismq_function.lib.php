 ...
 * @return	void
 */
function dolismqHeader($head = '', $title = '', $help_url = '', $target = '', $disablejs = 0, $disablehead = 0, $arrayofjs = '', $arrayofcss = '', $morequerystring = '', $morecssonbody = '', $replacemainareaby = '')
{
	global $conf, $langs, $db, $user;

	require_once __DIR__ . '/../class/dolismqelement.class.php';
	require_once __DIR__ . '/../core/modules/dolismq/dolismqelement/groupment/mod_groupment_standard.php';
	require_once __DIR__ . '/../core/modules/dolismq/dolismqelement/workunit/mod_workunit_standard.php';
	...
	<?php require_once './../../core/tpl/dolismq_medias_gallery_modal.tpl.php'; ?>
	...
	<div id="id-container" class="id-container page-ut-gp-list">
		<div class="side-nav">
			...
			<div id="id-left">
				<div class="dolismq-wrap wpeo-wrap">
					<div class="navigation-container">
						<div class="society-header">
							<a class="linkElement" href="../dolismqstandard/dolismqstandard_card.php?id=<?php echo $conf->global->DIGIRISKDOLIBARR_ACTIVE_STANDARD ?>">
								<span class="icon fas fa-building fa-fw"></span>
								<div class="title"><?php echo $conf->global->MAIN_INFO_SOCIETE_NOM ?></div>
								<?php if ($user->rights->dolismq->dolismqelement->write) : ?>
									<div class="add-container">
										<a id="newGroupment" href="../dolismqelement/dolismqelement_card.php?action=create&element_type=groupment&fk_parent=0">
										...
										<a id="newWorkunit" href="../dolismqelement/dolismqelement_card.php?action=create&element_type=workunit&fk_parent=0">
						...
						<ul class="workunit-list">
							...
							<script>
								...
								jQuery( '.dolismq-wrap .navigation-container .unit.active' ).removeClass( 'active' );

								var params = new window.URLSearchParams(window.location.search);
								var id = params.get('id');
								if (document.URL.match(/dolismqelement/) && !document.URL.match(/type=standard/)) {
...
function display_recurse_tree($results)
{
	...
	if ( ! $error && $action == "addDigiriskElementFiles") {
		...
		$dolismqelement_id = $data['dolismqelement_id'];
		$filenames          = $data['filenames'];

		$dolismqelement = new DigiriskElement($db);
		$dolismqelement->fetch($dolismqelement_id);
		$pathToDigiriskElementPhoto = $conf->dolismq->multidir_output[$conf->entity] . '/' . $dolismqelement->element_type . '/' . $dolismqelement->ref ;
		$filenames                  = preg_split('/vVv/', $filenames);
		array_pop($filenames);

		if ( ! (empty($filenames))) {
			$dolismqelement->photo = $filenames[0];
			foreach ($filenames as $filename) {
				if (is_file($conf->ecm->multidir_output[$conf->entity] . '/dolismq/medias/' . $filename)) {
					...
					if ( ! is_dir($conf->dolismq->multidir_output[$conf->entity] . '/' . $dolismqelement->element_type . '/')) {
						mkdir($conf->dolismq->multidir_output[$conf->entity] . '/' . $dolismqelement->element_type . '/');
					}
	...
	if ( ! $error && $action == "addDigiriskElementPhotoToFavorite") {
		...
		$dolismqelement = new DigiriskElement($db);
		$dolismqelement->fetch($dolismqelement_id);
		$dolismqelement->photo = $filename;
		$dolismqelement->update($user, true);
	}
	...
	if ( ! $error && $action == "unlinkDigiriskElementFile") {
		...
		if ($dolismqelement->photo == $filename) {
			$dolismqelement->photo = '';
			$dolismqelement->update($user, true);
		}
	...
	if ($user->rights->dolismq->dolismqelement->read) {
		if ( ! empty($results)) {
			...
			<li class="unit type-<?php echo $element['object']->element_type; ?>" id="unit<?php  echo $element['object']->id; ?>">
				<div class="unit-container">
						...
						<?php }
						$pathToThumb = DOL_URL_ROOT . '/viewimage.php?modulepart=dolismq&entity=' . $conf->entity . '&file=' . urlencode($element['object']->element_type . '/' . $element['object']->ref . '/thumbs/');
						$filearray   = dol_dir_list($conf->dolismq->multidir_output[$conf->entity] . '/' . $element['object']->element_type . '/' . $element['object']->ref . '/', "files", 0, '', '(\.odt|_preview.*\.png)$', 'position_name', 'asc', 1);
						if (count($filearray)) {
							print '<span class="floatleft inline-block valignmiddle divphotoref open-medias-linked modal-open dolismq-element dolismq-element-' . $element['object']->id . '" value="' . $element['object']->id . '">';
							 print '<img width="50" height="50" class="photo clicked-photo-preview" src="' . DOL_URL_ROOT . '/viewimage.php?modulepart=dolismq&entity=' . $conf->entity . '&file=' . urlencode($element['object']->element_type . '/' . $element['object']->ref . '/thumbs/' . preg_replace('/\./', '_small.', $element['object']->photo)) . '" >';
							 print '<input type="hidden" class="filepath-to-dolismqelement" value="' . $pathToThumb . '"/>';
							 print '</span>';
						} else {
							$nophoto = '/public/theme/common/nophoto.png'; ?>
							<div class="open-media-gallery modal-open dolismqelement dolismq-element-<?php echo $element['object']->id ?>" value="<?php  echo $element['object']->id ?>">
								<input type="hidden" class="type-from" value="dolismqelement"/>
								<input type="hidden" class="filepath-to-dolismqelement" value="<?php echo $pathToThumb ?>"/>
						...
						<?php } ?>
					<div class="dolismq-element-medias-modal" style="z-index:1500" value="<?php echo $element['object']->id ?>">
							<div class="wpeo-modal"  id="dolismq_element_medias_modal_<?php echo $element['object']->id ?>" value="<?php echo $element['object']->id ?>" style="z-index: 1005 !important">
								<div class="modal-container wpeo-modal-event">
									...
									<div class="modal-content" id="#modalContent<?php echo $element['object']->id ?>">
										...
										<div class="risk-evaluation-container">
											...
											<div class="element-linked-medias element-linked-medias-<?php echo $element['object']->id ?> dolismq-element modal-media-linked">
												<div class="medias"><i class="fas fa-picture-o"></i><?php echo $langs->trans('Medias'); ?></div>
													<?php
													$relativepath = 'dolismq/medias/thumbs';
													print dolismq_show_medias_linked('dolismq', $conf->dolismq->multidir_output[$conf->entity] . '/' . $element['object']->element_type . '/', 'small', '', 0, 0, 0, 150, 150, 1, 0, 0, $element['object']->element_type, $element['object']);
													?>
											</div>
										</div>
									</div>
					...
					<div class="title" id="scores" value="<?php echo $element['object']->id ?>">
						<?php
						if ($user->rights->dolismq->risk->read) : ?>
							<a id="slider" class="linkElement id<?php echo $element['object']->id;?>" href="../dolismqelement/dolismqelement_risk.php?id=<?php echo $element['object']->id; ?>">
						...
						<?php else : ?>
							<a id="slider" class="linkElement id<?php echo $element['object']->id;?>" href="../dolismqelement/dolismqelement_card.php?id=<?php echo $element['object']->id; ?>">
								<span class="title-container">
									<span class="ref"><?php echo $element['object']->ref; ?></span>
									<span class="name"><?php echo $element['object']->label; ?></span>
								</span>
						...
						<?php if ($user->rights->dolismq->dolismqelement->write) : ?>
							...
							<div class="add-container">
								<a id="newGroupment" href="../dolismqelement/dolismqelement_card.php?action=create&element_type=groupment&fk_parent=<?php echo $element['object']->id; ?>">
									<div
								...
								<a id="newWorkunit" href="../dolismqelement/dolismqelement_card.php?action=create&element_type=workunit&fk_parent=<?php echo $element['object']->id; ?>">
									<div
 ...
 *  @return	void
 */
function dolismq_banner_tab($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '', $nodbprefix = 0, $morehtmlleft = '', $morehtmlstatus = '', $onlybanner = 0, $morehtmlright = '')
{
	global $form;
...
function dolismq_show_medias_linked($modulepart = 'ecm', $sdir, $size = 0, $nbmax = 0, $nbbyrow = 5, $showfilename = 0, $showaction = 0, $maxHeight = 120, $maxWidth = 160, $nolink = 0, $notitle = 0, $usesharelink = 0, $subdir = "", $object = null)
{
		global $conf, $user, $langs;
 ...
 */
function dolismq_selectcontacts($socid, $selected = '', $htmlname = 'contactid', $showempty = 0, $exclude = '', $limitto = '', $showfunction = 0, $moreclass = '', $options_only = false, $showsoc = 0, $forcecombo = 0, $events = array(), $moreparam = '', $htmlid = '', $multiple = false, $disableifempty = 0, $exclude_already_add = '')
{
 ...
 * 	@return	void
 */
function dolismq_select_dictionary($htmlname, $dictionarytable, $keyfield = 'code', $labelfield = 'label', $selected = '', $useempty = 0, $moreattrib = '')
{
	// phpcs:enable
