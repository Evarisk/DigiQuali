...
$res = $productlot->fetch_optionals();

$head = productlot_prepare_head($productlot);
print dol_get_fiche_head($head, 'controls', $langs->trans("Controls"), -1, $productlot->picto);

$formconfirm = '';

// Confirmation to delete
if ($action == 'delete') {
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$productlot->id, $langs->trans('DeleteBatch'), $langs->trans('ConfirmDeleteBatch'), 'confirm_delete', '', 0, 1);
}

// Call Hook formConfirm
$parameters = array('formConfirm' => $formconfirm, 'lineid' => $lineid);
$reshook = $hookmanager->executeHooks('formConfirm', $parameters, $productlot, $action); // Note that $action and $object may have been modified by hook
if (empty($reshook)) {
...
$producttmp->fetch($productlot->fk_product);
...
else $sql .= " WHERE 1 = 1 AND fk_lot =".$id;
...
$newcardbutton = dolGetButtonTitle($langs->trans('NewControl'), '', 'fa fa-plus-circle', dol_buildpath('/dolismq/view/control/control_card.php', 1).'?action=create&fk_product='.$productlot->fk_product.'&fk_lot='.$productlot->id.'&backtopage='.urlencode($_SERVER['PHP_SELF']. '?id='.$productlot->id),'', $permissiontoadd);
