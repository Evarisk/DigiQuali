...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'question_grouplist'; // To manage different context of search
...
$hookmanager->initHooks(array('question_grouplist')); // Note that conf->hooks_modules contains array
...
if (!empty($conf->categorie->enabled)) {
	$search_category_array = GETPOST("search_category_question_group_list", "array");
}
...
$permissiontoadd    = $user->rights->digiquali->question_group->write;
$permissiontodelete = $user->rights->digiquali->question_group->delete;
...
if (empty($reshook)) {
		...
		if (!empty($toselect)) {
			foreach ($toselect as $toselectedid) {
                ...
                if ($object->type == 'UniqueChoice' || $object->type == 'MultipleChoices') {
                        $answerList = $answer->fetchAll('ASC', 'position', 0, 0, ['fk_question_group' => $object->id]);
                }
    ...
    if (!$error && ($massaction == 'add_question_groups' || ($action == 'add_question_groups' && $confirm == 'yes')) && $permissiontoadd) {
        if (is_array($toselect) && !empty($toselect)) {
            $totalQuestions  = 0;
            $question_groupInArray = [];
            if (GETPOSTISSET('sheet') && GETPOST('sheet') > 0) {
                ...
                foreach ($toselect as $selected) {
                    ...
                    if (is_array($sheet->linkedObjectsIds['digiquali_' . $object->element]) && !empty($sheet->linkedObjectsIds['digiquali_' . $object->element]) && in_array($object->id, $sheet->linkedObjectsIds['digiquali_' . $object->element])) {
                        $question_groupInArray[] = $object->getNomUrl(1, 'nolink', 1);
                    } else {
                        $totalQuestions++;
                        $object->add_object_linked('digiquali_' . $sheet->element, GETPOST('sheet'));
                        $question_groupIds   = $sheet->linkedObjectsIds['digiquali_' . $object->element];
                        $question_groupIds[] = $object->id;
                        $sheet->updateQuestionsPosition($question_groupIds);
                    }
                }
                if (!empty($question_groupInArray)) {
                    setEventMessages($langs->trans('WarningQuestionLink', count($question_groupInArray)) . ' ', $question_groupInArray, 'warnings');
                }
                if ($totalQuestionGroups > 0) {
                    setEventMessages($langs->trans('AddQuestionGroupLink', $totalQuestionGroups) . ' ' . $sheet->getNomUrl(1), []);
                }
            } else {
...
$now      = dol_now();
$help_url = '';
$title    = $langs->trans('ListOf', $langs->transnoentitiesnoconv("QuestionGroups"));
...
if (!empty($conf->categorie->enabled)) {
	$sql .= Categorie::getFilterJoinQuery('question_group', "t.rowid");
}
...
foreach ($search as $key => $val) {
    if ($key == 'status' && $val == 'specialCase') {
        $newStatus = [QuestionGroup::STATUS_VALIDATED, QuestionGroup::STATUS_LOCKED];
        if (!empty($newStatus)) {
...
if (!empty($conf->categorie->enabled)) {
	$sql .= Categorie::getFilterSelectQuery('question_group', "t.rowid", $search_category_array);
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $searchAll && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/digiquali/view/questiongroup/questiongroup_card.php', 1).'?id='.$id);
	exit;
}
...
$arrayofmassactions['pre_add_question_groups'] = '<span class="fas fa-plus-circle paddingrightonly"></span>' . $langs->transnoentities('AddToSheet');
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/digiquali/view/questiongroup/questiongroup_card.php', 1).'?action=create', '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

if ($massaction == 'prelock') {
    print $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('ConfirmMassLock'), $langs->trans('ConfirmMassLockingQuestionGroup', count($toselect)), 'lock', null, '', 0, 200, 500, 1);
}

if ($massaction == 'prearchive') {
    print $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('ConfirmMassArchive'), $langs->trans('ConfirmMassArchivingQuestionGroup', count($toselect)), 'archive', null, '', 0, 200, 500, 1);
}

if ($massaction == 'pre_add_question_groups') {
    ...
    if (is_array($sheets) && !empty($sheets)) {
        ...
        print $form->formconfirm($_SERVER['PHP_SELF'], $langs->trans('ConfirmMassAddQuestionGroup'), $langs->trans('ConfirmMassAddingQuestionGroup', count($toselect)), 'add_question_groups', $formQuestionGroup, '', 0, 200, 500, 1);
    } else {
...
if (!empty($conf->categorie->enabled) && $user->rights->categorie->lire && !empty($search_category_array)) {
	$formcategory  = new FormCategory($db);
	$moreforfilter .= $formcategory->getFilterBox('question_group', $search_category_array);
}
