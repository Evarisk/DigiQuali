...
/**
 *  \file       view/sheet/sheet_agenda.php
 *  \ingroup    dolismq
 *  \brief      Page of Sheet events
 */
...
if ($object->id > 0) {
	...
	print dol_get_fiche_head($head, 'sheetAgenda', $title, -1, "dolismq@dolismq");

	// Object card
	// ------------------------------------------------------------
	dol_strlen($object->label) ? $morehtmlref = ' - ' . $object->label : '';
	dol_banner_tab($object, 'ref', '', 0, 'ref', 'ref', $morehtmlref, '', 0, '', $object->getLibStatut(5));
	...
	if ( ! empty($conf->agenda->enabled) && ( ! empty($user->rights->agenda->myactions->read) || ! empty($user->rights->agenda->allactions->read))) {
		...
		print_barre_liste($langs->trans("ActionsOnSheet"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlcenter, '', 0, 1, 1);
