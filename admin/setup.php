...
$arrayofparameters = array(
	'DOLISMQ_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'DOLISMQ_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/dolismq/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOLISMQ_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOLISMQ_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_dolismq@dolismq');
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'DOLISMQ_'.strtoupper($myTmpObjectKey).'_ADDON';
