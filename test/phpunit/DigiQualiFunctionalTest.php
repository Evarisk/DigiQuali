...
/**
 * \file    test/phpunit/DigiQualiFunctionalTest.php
 * \ingroup digiquali
 ...
 *
 * @package Testdigiquali
 */
class DigiQualiFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modDigiQuali')->click();
		} else {
			// Disable the module
			$this->byHref('modDigiQuali')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/digiquali/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('digiquali/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/digiquali/admin/about.php');
		$this->authenticate();
		return $this->assertContains('digiquali/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('digiqualiwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modDigiQuali_DigiQualiTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modDigiQuali_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
