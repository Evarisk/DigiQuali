 ...
 */
class pdf_calypso extends ModeleODTControlDocument
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->dolismq->dir_output)
		{
			...
			if ($object->specimen)
			{
				$dir = $conf->dolismq->dir_output;
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->dolismq->multidir_output[$conf->entity]."/controldocument/".$objectref;
				$date = dol_print_date(dol_now(),'dayxcard');
				$filename = $object->ref.'_'.$date.'.pdf';
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("ControlCard")." ".$outputlangs->convToOutputCharset($object->thirdparty->name));
...
////			$tempdir = $conf->dolismq->multidir_output[isset($object->entity) ? $object->entity : 1] . '/temp/';
////
////			//Signatures
...
//	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("ControlDocument");
