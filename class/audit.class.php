...
class Audit extends CommonObject
{
	 ...
	 */
	public $table_element = 'dolismq_audit';
	...
	public $fields=array(
		...
		'date_start' => array('type'=>'datetime', 'label'=>'DateStart', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>-1, 'help'=>"DoliSMQHelpAuditDateStart", 'comment'=>"Date de début d'audit"),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'dolismq_auditline';
	...
	//  */
	// public $class_element_line = 'Auditline';
	...
	//  */
	// protected $childtablesoncascade = array('dolismq_auditdet');

	// /**
	//  * @var AuditLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->dolismq->audit->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolismq->audit->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolismq->audit->audit_advance->validate))))
		 {
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$dirsource = $conf->dolismq->dir_output.'/audit/'.$oldref;
				$dirdest = $conf->dolismq->dir_output.'/audit/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->dolismq->dir_output.'/audit/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Audit").'</u>';
		...
		$url = dol_buildpath('/dolismq/audit_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_audit = '.$this->id));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->DOLISMQ_AUDIT_ADDON)) {
			$conf->global->DOLISMQ_AUDIT_ADDON = 'mod_audit_standard';
		}
...
/**
 * Class AuditLine. You can also remove this and generate a CRUD class for lines objects.
 */
class AuditLine extends CommonObjectLine
{
	// To complete with content of an object AuditLine
	// We should have a field rowid, fk_audit and position
